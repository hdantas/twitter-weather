...
-- file = LOAD '../data/AllTweets_grid.txt' USING PigStorage('\t') AS (count:long,userID:long,userName:chararray,messageID:long,date:chararray,gps_lat:double,gps_long:double,source:chararray,tweet:chararray,gpsblock:chararray);

file = LOAD '$input_file' USING PigStorage('\t') AS (count:long,userID:long,userName:chararray,messageID:long,date:chararray,gps_lat:double,gps_long:double,source:chararray,tweet:chararray,gpsblock:chararray);
...
STORE ordered_flows INTO '$output_dir'; --write the result on Disk. $output_dir is a command line argument
